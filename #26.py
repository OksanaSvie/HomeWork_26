# Домашнее задание 26
# На основании того, что проходили на паре по Selenium WebDriver,
# написать программу которая заходит на сайт mensa, и проходит тест 5 разными способами.

from selenium import webdriver
import time

#1
driver = webdriver.Chrome()
driver.maximize_window()

url = 'https://test.mensa.no/'

driver.get(url)

xpath_button_18_50 = '/html/body/div[2]/main/cach/div[1]/div/div/div[2]/div/button[2]'
button_18_50 = driver.find_element('xpath',xpath_button_18_50)
button_18_50.click()

xpath_button_start = '/html/body/div[2]/main/cach/div[2]/div/div/div/div[2]/button'
button_start = driver.find_element('xpath',xpath_button_start)
button_start.click()
time.sleep(2)


for i in range(0,35):
    xpath_button_q_a1 = f'//div[@id="question_{i}"]//div[@data-answerid="0"]'
    button_q_a1 = driver.find_element('xpath', xpath_button_q_a1)
    button_q_a1.click()
    time.sleep(2)

xpath_button_finish = '/html/body/div[2]/main/cach/div[3]/div[35]/div[5]/div/button[3]'
button_finish = driver.find_element('xpath',xpath_button_finish)
button_finish.click()
time.sleep(2)

xpath_button_finish_2 = '/html/body/div[2]/main/cach/div[6]/div/div/div[3]/button[2]'
button_finish_2 = driver.find_element('xpath',xpath_button_finish_2)
button_finish_2.click()


# #2
# driver = webdriver.Chrome()
# driver.maximize_window()
#
#
# driver.get(url)
#
# xpath_button_18_50 = '/html/body/div[2]/main/cach/div[1]/div/div/div[2]/div/button[2]'
# button_18_50 = driver.find_element('xpath',xpath_button_18_50)
# button_18_50.click()
# time.sleep(2)
#
#
# xpath_button_start = '/html/body/div[2]/main/cach/div[2]/div/div/div/div[2]/button'
# button_start = driver.find_element('xpath',xpath_button_start)
# button_start.click()
# time.sleep(2)
#
#
# for i in range(1,35+1):
#     a = 2
#     xpath_button_q_a1 = f'/html/body/div[2]/main/cach/div[3]/div[{i}]/div[3]/div[{a}]/div/img'
#     button_q_a1 = driver.find_element('xpath', xpath_button_q_a1)
#     button_q_a1.click()
#     time.sleep(2)
#
#
# xpath_button_finish = '/html/body/div[2]/main/cach/div[3]/div[35]/div[5]/div/button[3]'
# button_finish = driver.find_element('xpath',xpath_button_finish)
# button_finish.click()
# time.sleep(2)
#
# xpath_button_finish_2 = '/html/body/div[2]/main/cach/div[6]/div/div/div[3]/button[2]'
# button_finish_2 = driver.find_element('xpath',xpath_button_finish_2)
# button_finish_2.click()
#
#
# #3
# driver = webdriver.Chrome()
# driver.maximize_window()
#
# driver.get(url)
#
# xpath_button_18_50 = '/html/body/div[2]/main/cach/div[1]/div/div/div[2]/div/button[2]'
# button_18_50 = driver.find_element('xpath',xpath_button_18_50)
# button_18_50.click()
#
# xpath_button_start = '/html/body/div[2]/main/cach/div[2]/div/div/div/div[2]/button'
# button_start = driver.find_element('xpath',xpath_button_start)
# button_start.click()
# time.sleep(2)
#
# for i in range(1,35+1):
#     a = 3
#     xpath_button_q_a1 = f'/html/body/div[2]/main/cach/div[3]/div[{i}]/div[3]/div[{a}]/div/img'
#     button_q_a1 = driver.find_element('xpath', xpath_button_q_a1)
#     button_q_a1.click()
#     time.sleep(2)
#
#
# xpath_button_finish = '/html/body/div[2]/main/cach/div[3]/div[35]/div[5]/div/button[3]'
# button_finish = driver.find_element('xpath',xpath_button_finish)
# button_finish.click()
# time.sleep(2)
#
# xpath_button_finish_2 = '/html/body/div[2]/main/cach/div[6]/div/div/div[3]/button[2]'
# button_finish_2 = driver.find_element('xpath',xpath_button_finish_2)
# button_finish_2.click()
#
#
# #4
# driver = webdriver.Chrome()
# driver.maximize_window()
#
# driver.get(url)
#
# xpath_button_18_50 = '/html/body/div[2]/main/cach/div[1]/div/div/div[2]/div/button[2]'
# button_18_50 = driver.find_element('xpath',xpath_button_18_50)
# button_18_50.click()
#
# xpath_button_start = '/html/body/div[2]/main/cach/div[2]/div/div/div/div[2]/button'
# button_start = driver.find_element('xpath',xpath_button_start)
# button_start.click()
# time.sleep(2)
#
# for i in range(1,35+1):
#     a = 4
#     xpath_button_q_a1 = f'/html/body/div[2]/main/cach/div[3]/div[{i}]/div[3]/div[{a}]/div/img'
#     button_q_a1 = driver.find_element('xpath', xpath_button_q_a1)
#     button_q_a1.click()
#     time.sleep(2)
#
#
# xpath_button_finish = '/html/body/div[2]/main/cach/div[3]/div[35]/div[5]/div/button[3]'
# button_finish = driver.find_element('xpath',xpath_button_finish)
# button_finish.click()
# time.sleep(2)
#
# xpath_button_finish_2 = '/html/body/div[2]/main/cach/div[6]/div/div/div[3]/button[2]'
# button_finish_2 = driver.find_element('xpath',xpath_button_finish_2)
# button_finish_2.click()
#
# #5
# driver = webdriver.Chrome()
# driver.maximize_window()
#
# driver.get(url)
#
# xpath_button_18_50 = '/html/body/div[2]/main/cach/div[1]/div/div/div[2]/div/button[2]'
# button_18_50 = driver.find_element('xpath',xpath_button_18_50)
# button_18_50.click()
#
# xpath_button_start = '/html/body/div[2]/main/cach/div[2]/div/div/div/div[2]/button'
# button_start = driver.find_element('xpath',xpath_button_start)
# button_start.click()
# time.sleep(2)
#
# for i in range(1,35+1):
#     a = 5
#     xpath_button_q_a1 = f'/html/body/div[2]/main/cach/div[3]/div[{i}]/div[3]/div[{a}]/div/img'
#     button_q_a1 = driver.find_element('xpath', xpath_button_q_a1)
#     button_q_a1.click()
#     time.sleep(2)
#
#
# xpath_button_finish = '/html/body/div[2]/main/cach/div[3]/div[35]/div[5]/div/button[3]'
# button_finish = driver.find_element('xpath',xpath_button_finish)
# button_finish.click()
# time.sleep(2)
#
# xpath_button_finish_2 = '/html/body/div[2]/main/cach/div[6]/div/div/div[3]/button[2]'
# button_finish_2 = driver.find_element('xpath',xpath_button_finish_2)
# button_finish_2.click()
#
